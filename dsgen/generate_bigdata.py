...
# constraints. It writes to records to disk, and generates a proportional 
# number of duplicates at regular intervals of a million original records 
# (approx 100mb). The catch is that the output is not randomly sorted 
...
def parse_duplicates(num_dup_records, org_rec, num_to_duplicate):
  ...
  while (rec_cnt < num_to_duplicate):
    rand_rec_num = random.randint(0, 999999)
    ...
    #
    while (d < num_dups) and (rec_cnt < num_to_duplicate):
      ...
      if (rec_str not in all_rec_set):  # Check if same record already created
        ...
        for field_name in field_name_list:
          out_line = out_line + dup_rec_dict.get(field_name, missing_value) + ', '
...
print
print 'Step 2a: Open output file'
print
...
while (rec_cnt < num_org_records):
  ...
  if (rec_cnt < 1000000):  # Check if same record already created
    all_rec_set.add(rec_str)
  ...
  # Write to file after creating each record
  rec_cnt += 1
  out_line = ''
  for field_name in field_name_list:
  ...
  # Parse duplicates after every million records

  size = len(org_rec)
  if (size == 1000000):
    # duplicate a suitable proportion
    num_to_duplicate = math.floor(num_org_records/1000000)
    num_to_duplicate = math.floor(num_dup_records/num_to_duplicate)
    ...
    rec_dict.clear()
    rec_cnt = 0
    num_org_records = num_org_records - 1000000
    num_dup_records = num_dup_records - num_to_duplicate
    size = 0
...
while (rec_cnt < num_dup_records):
  ...
  # this will break the distribution because it is not expected behaviour in the original febrl this would have been an infinite loop ;) 
    ...
    facade_rec_num = rand_rec_num
  else: 
    apriori_rec_num = rec_cnt
    facade_rec_num = apriori_rec_num
  org_rec_id = 'rec-%i-org' % (facade_rec_num)
  ...
  while (d < num_dups) and (rec_cnt < num_dup_records):
    ...
    #
    dup_rec_dict = org_rec_dict.copy()  # Make a copy of the original record
    dup_rec_id =             'rec-%i-dup-%i' % (facade_rec_num, d)
    dup_rec_dict['rec_id'] = dup_rec_id
    ...
    if (rec_str not in all_rec_set):  # Check if same record already created
      ...
      # Process the duplicate records now 
      out_line = ''
      for field_name in field_name_list:
        out_line = out_line + dup_rec_dict.get(field_name, missing_value) + ', '
        if (field_name == 'rec_id') and (out_line[-6:] == '-org, '):
...
# -----------------------------------------------------------------------------
# Write output csv file (Not necessary, since already written)
#
